...
from backend import Generate, CustomMessage
...
def js0nime():
    ...
    """
        ...
        if all_episodes != 1:
            while True:
                choice = input(
                    f"{IN}Do you want to generate all episode? (y/n) > "
                ).lower()
                if choice in ["y", "n"]:
                    break
                else:
        ...
        print(
            f"{OK}Generating {Fore.LIGHTCYAN_EX}{len(download_urls)}{Fore.RESET} episode/s"
        )
